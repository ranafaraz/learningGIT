...
<div class="rh_login_modal_wrapper <?php echo esc_attr($form_design); ?>">
	<div class="rh_login_modal_box <?php if (!('true' == $inspiry_login_quote_side_display)) {
		...
		if ('true' == $inspiry_login_quote_side_display) {
			?>
			<div class="rh_login_sides rh_login_quote_side" <?php if (!empty(get_option('theme_login_modal_background'))) {
				?> style="background-image: url('<?php echo esc_url(get_option('theme_login_modal_background')); ?>')" <?php
			} ?>>
		...
		<div class="rh_login_sides rh_login_form_side">
			...
			<div class="rh_wrapper_login_forms">
				<div class="rh_form_modal rh_login_form rh_login_modal_show">
					...
					<form id="rh_modal__login_form" action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="POST"
						enctype="multipart/form-data">
						<label class="rh_modal_labels" for="username"><?php echo esc_html($label_user_name) ?></label>
						<input name="log" class="rh_modal_field focus-class" autocomplete="username" id="username" type="text"
						...
						<div class="rh_wrapper_password_field">
							<i class="fa fa-eye rh_password_reveal_trigger"></i>
							<input name="pwd" class="rh_modal_field" autocomplete="current-password" id="password" type="password"
						...
						if (class_exists('Easy_Real_Estate')) {
							if (ere_is_reCAPTCHA_configured()) {
								...
								if (
									!is_page_template('templates/contact.php') && empty(get_option('inspiry_contact_form_shortcode'))
								) {
									?>
						...
						if (!empty($inspiry_login_button_text)) {
							?>
							<button id="login-button" type="submit"><?php echo esc_html($inspiry_login_button_text); ?></button>
							<?php
						} else {
							?>
				...
				if ($users_can_register) {
					...
					<div class="rh_form_modal rh_register_form">
						<form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" id="rh_modal__register_form" method="post"
							...
							<label class="rh_modal_labels" for="register_email"><?php echo esc_html($label_email) ?></label>
								...
								foreach ($user_fields as $field) {
									...
									if (!empty($field['id']) && !empty($field['name'])) {
										...
										if (!empty($field['type']) && $field['type'] == 'select' && !empty($field['options']) && is_array($field['options'])) {
											...
											?>
												<input class="rh_modal_input_cb" type="checkbox" id="<?php echo esc_attr($field['id']); ?>"
													name="<?php echo esc_attr($field['id']); ?>" value="<?php echo esc_attr($field['std']); ?>">
												<label class="rh_modal_label_cb"
													for="<?php echo esc_attr($field['id']); ?>"><span><?php echo esc_attr($field['title']); ?></span></label>
											<?php
										} else {
											?>
												<label class="rh_modal_labels"
							...
							if ('true' == $user_sync) {
								...
								if (!empty($user_roles) && is_array($user_roles)) {
									...
									?>
									<label class="rh_modal_labels" for="register_username"><?php echo esc_html($user_role_label); ?></label>
									<div class="rh_user_role rh_modal_role_select">
										<select name="user_role" id="user-role"
											class="inspiry_select_picker_trigger inspiry_bs_default_mod  inspiry_bs_green show-tick dropup"
							...
							if (class_exists('Easy_Real_Estate')) {
								if (
									ere_is_reCAPTCHA_configured() && !is_page_template('templates/contact.php')
								) {
									?>
									<div class="rh_modal__recaptcha">
										<div
											class="inspiry-recaptcha-wrapper tt clearfix g-recaptcha-type-<?php echo esc_attr($recaptcha_type); ?>">
											<div class="inspiry-google-recaptcha"></div>
										</div>
									</div>
									<?php
			...
			<div
				class="rh_modal_login_loader rh_modal_login_loader_hide rh_modal_login_<?php echo esc_attr(INSPIRY_DESIGN_VARIATION); ?>">
