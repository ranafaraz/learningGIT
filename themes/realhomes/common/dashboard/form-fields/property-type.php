...
<p>
    ...
    <select name="type[]" id="type" class="inspiry_select_picker_trigger show-tick" data-size="5"
        data-actions-box="true" <?php
        ...
        if ('yes' == $inspiry_search_form_multiselect_types) {
            ?> data-selected-text-format="count > 2" multiple="multiple"
            data-count-selected-text="{0} <?php esc_attr_e(' Types Selected ', 'framework'); ?>" <?php
        }
        ?>
        title="<?php esc_attr_e('None', 'framework'); ?>" required>
        <?php
