...
if (!function_exists('inspiry_social_login_links')):
	function inspiry_social_login_links($provider_id, $provider_name, $authenticate_url)
	{
		...
		<a rel="nofollow" href="<?php echo esc_url($authenticate_url); ?>" data-provider="<?php echo esc_attr($provider_id); ?>"
			...
			?>
			<span><i class="<?php echo esc_attr(strtolower($provider_id)); ?>"></i>
				<?php echo esc_html($provider_name); ?></span>
		</a>
		<?php
	}
...
if (!function_exists('inspiry_insert_role_user')) {
	...
	function inspiry_insert_role_user($post_id, $post)
	{
		...
		$email_address = get_post_meta($post_id, 'REAL_HOMES_agent_email', true);
		...
		if ($agency_id) {
			update_user_meta($user_id, 'inspiry_user_agency', $agency_id);
		}
		...
		if (function_exists('ere_new_user_notification')) {
			ere_new_user_notification($user_id, $user_pass);
		}
	...
	function inspiry_add_user_agency_field($user)
	{
		...
		<table class="form-table">
			...
			<tr class="inspiry-user-address-wrap">
				...
				<td><textarea name="inspiry_user_address" id="inspiry_user_address" rows="3"
						cols="30"><?php echo get_the_author_meta('inspiry_user_address', $user->ID); ?></textarea></td>
			</tr>
		</table>
		<?php
	}
				...
				<td>
					...
					<select name="realhomes_user_approval_field" id="realhomes-user-approval-field" <?php echo $is_current_user ? 'disabled' : ''; ?>>
						...
						foreach ($all_statuses as $key => $status) {
							?>
							<option value="<?php echo esc_attr($key); ?>" <?php selected($current_user_status, $key, true); ?>>
