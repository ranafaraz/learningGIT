...
if (!function_exists('realhomes_dashboard_submit_property')) {
  ...
  function realhomes_dashboard_submit_property()
  {
    ...
    if (empty($user_phone)) {
      $response['message'] = esc_html__('You must fill in a phone number on the profile page', 'framework');
      wp_send_json_error($response);
    }
    ...
    if (
      ...
      (!isset($_POST['agent_display_option']) || empty(sanitize_text_field(trim($_POST['agent_display_option']))) ||
        ($_POST['agent_display_option'] === 'agent_info' && (!isset($_POST['agent_id']) || empty($_POST['agent_id']))))
    ) {
    ...
    if (isset($_POST['agent_display_option']) && $_POST['agent_display_option'] === 'agent_info') {
      ...
      if ($user_role == 'agency') {
        $agency_id = get_user_meta($user_id, 'inspiry_role_post_id', true);
      }
...
if (!function_exists('dirabe_realhomes_property_id_duplicate_search')) {
  function dirabe_realhomes_property_id_duplicate_search()
  {
    $response = array();

    // Verify user is logged in
    if (!is_user_logged_in()) {
      $response['message'] = esc_html__('You must be logged in to perform this action!', 'framework');
      wp_send_json_error($response);
    }

    // Verify required field (Property ID)
    if (!isset($_POST['property_id']) || empty(sanitize_text_field(trim($_POST['property_id'])))) {
      $response['message'] = esc_html__('Property id field is required!', 'framework');
      wp_send_json_error($response);
    }

    // Get all possible authors
    $property_id = sanitize_text_field(trim($_POST['property_id']));
    $user_id = get_current_user_id();
    $user_role = get_user_meta($user_id, 'inspiry_user_role', true);
    $authors = array($user_id);
    if ($user_role === 'agent') {
      $user_agency_id = get_user_meta($user_id, 'inspiry_user_agency', true);
      if ($user_agency_id) {
        $agency_user_id = get_users(
          array(
            'meta_key' => 'inspiry_user_role',
            'meta_value' => $user_agency_id,
            'fields' => 'ID',
          )
        );
        $agency_agents = get_users(
          array(
            'meta_key' => 'inspiry_user_agency',
            'meta_value' => $user_agency_id,
            'fields' => 'ID',
        ...
        $authors = array_merge($agency_user_id, $agency_agents);
      }
    } else if ($user_role === 'agency') {
      $user_agency_id = get_user_meta($user_id, 'inspiry_role_post_id', true);
      if ($user_agency_id) {
        $agency_agents = get_users(
          array(
            'meta_key' => 'inspiry_user_agency',
        ...
        $authors = array_merge($authors, $agency_agents);
      }
    }

    // Search for property with the same ID and author (limit to 1) and not same post
    $additional_args = array();

    $post_id = isset($_POST['post_id']) ? intval($_POST['post_id']) : null;
    if ($post_id) {
      $additional_args['exclude'] = array($post_id);
    }
    ...
    $property = get_post($property_ids[0]);
    if (empty($property)) {
      wp_send_json_success($response);
    }

    if (!empty($property)) {
      $response['message'] = esc_html__('Property with this ID already exists!', 'framework');
      wp_send_json_error($response);
    }
  ...
  add_action('wp_ajax_dirabe_realhomes_property_id_duplicate_search', 'dirabe_realhomes_property_id_duplicate_search');
}
...
if (!function_exists('realhomes_dashboard_trash_post')) {
  ...
  function realhomes_dashboard_trash_post()
  {
    ...
    $allowed_post_types = ['property']; //, 'agent'];
    ...
    if (!in_array($user_id, $allowedUsers)) {
      echo json_encode(
        array(
          'success' => false,
          'message' => sprintf(esc_html__("You don't have permission to delete this %s.", 'framework'), ucfirst($post_type))
        )
      );
      wp_die();
    }
...
if (!function_exists('realhomes_dashboard_assets')) {
  ...
  function realhomes_dashboard_assets()
  {
    ...
    // Enqueue Lodash from a CDN
    wp_enqueue_script('lodash', 'https://cdn.jsdelivr.net/npm/lodash@4.17.21/lodash.min.js', array(), null, true);
