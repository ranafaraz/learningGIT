...
if (!function_exists('inspiry_hide_empty_locations')):
	...
	{
		if (
			('true' == get_option('inspiry_hide_empty_locations', 'true'))
...
if (!function_exists('realhomes_hide_empty_property_types')):
	...
	function realhomes_hide_empty_property_types(): bool
	{
		...
		if ('true' == get_option('realhomes_hide_empty_property_types')) {
			return true;
		}
...
if (!function_exists('realhomes_locations_js_data') && 'yes' != get_option('inspiry_ajax_location_field')) {
	...
	function realhomes_locations_js_data()
	{
		...
		if (!is_admin() && class_exists('ERE_Data')) {
			...
			if (
				'yes' == get_option('inspiry_search_form_multiselect_locations', 'yes')
...
if (!function_exists('inspiry_floor_search')) {
	function inspiry_floor_search($meta_query)
	{
		if (isset($_GET['min-floor']) && (!$_GET['min-floor'] != inspiry_any_value()) && isset($_GET['max-floor']) && (!$_GET['max-floor'] != inspiry_any_value())) {
			$min_floor = doubleval($_GET['min-floor']);
			$max_floor = doubleval($_GET['max-floor']);
			if ($min_floor >= 0 && $max_floor > $min_floor) {
				$meta_query[] = array(
					'key' => 'REAL_HOMES_property_floor',
					'value' => array($min_floor, $max_floor),
					'type' => 'NUMERIC',
					'compare' => 'BETWEEN',
				);
			}
		} else if (isset($_GET['min-floor']) && ($_GET['min-floor'] != inspiry_any_value())) {
			$min_floor = doubleval($_GET['min-floor']);
			if ($min_floor > 0) {
				$meta_query[] = array(
					'key' => 'REAL_HOMES_property_floor',
		...
		} else if (isset($_GET['max-floor']) && ($_GET['max-floor'] != inspiry_any_value())) {
			$max_floor = doubleval($_GET['max-floor']);
			if ($max_floor > 0) {
				$meta_query[] = array(
					'key' => 'REAL_HOMES_property_floor',
	...
	add_filter('inspiry_real_estate_meta_search', 'inspiry_floor_search');
}
