...
if (!function_exists('inspiry_update_profile')) {
	...
	function inspiry_update_profile()
	{
		...
		if (wp_verify_nonce($_POST['user_profile_nonce'], 'update_user')) {
			...
			if (!isset($_POST['display-name']) || empty(trim($_POST['display-name']))) {
				$response['errors'][] = esc_html__('Display name is required!', 'framework');
				echo json_encode($response);
				die;
			}

			if (!isset($_POST['profile-image-id']) || empty(trim($_POST['profile-image-id']))) {
				$response['errors'][] = esc_html__('Profile image is required!', 'framework');
				echo json_encode($response);
				die;
			}

			if (!isset($_POST['email']) || empty(sanitize_email(trim($_POST['email']))) || !is_email(sanitize_email(trim($_POST['email'])))) {
				$response['errors'][] = esc_html__('Email is required!', 'framework');
				echo json_encode($response);
				die;
			}

			if (!isset($_POST['mobile-number']) || empty(trim($_POST['mobile-number']))) {
				$response['errors'][] = esc_html__('Mobile number is required!', 'framework');
				echo json_encode($response);
				die;
			}
