...
} elseif ( ! empty( $realhomes_custom_search_form_margin_top ) ) {
...
} elseif ( ! empty( $realhomes_custom_search_form_margin_bottom ) ) {
...
if ( '1' !== get_post_meta( $page_id, 'REAL_HOMES_hide_advance_search', true ) && inspiry_show_header_search_form() ) {
	...
	if ( class_exists( 'RHEA_Elementor_Search_Form' ) && ( 'default' !== $realhomes_custom_search_form || ( ! empty( $REAL_HOMES_custom_search_form ) && 'default' !== $REAL_HOMES_custom_search_form ) ) ) {
        ...
        <div class="rh-custom-search-form-wrapper rhea-hide-before-load"
             style="margin-top: <?php echo esc_attr( $margin_top ); ?>; margin-bottom:  <?php echo esc_attr( $margin_bottom ) ?>">
			<?php
