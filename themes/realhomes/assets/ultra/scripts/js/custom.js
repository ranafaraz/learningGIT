(function ($) {
  ...
  // Handle password reveal trigger click in login form.
  $('body').on('click', '#rh_modal__login_form .rh_wrapper_password_field .rh_password_reveal_trigger', function (e) {
    e.preventDefault();

    const passwordField = $('#rh_modal__login_form .rh_wrapper_password_field input[name="pwd"]');
  ...
  function waitForLodash(callback, timeout = 300, interval = 50) {
    ...
    (function checkLodash() {
      if (typeof _ !== 'undefined') {
        ...
        // Keep checking
        setTimeout(checkLodash, interval);
      } else {
        // Timeout
        console.error('Lodash did not load within the expected time.');
      }
    })();
  }

  // Check if property id field already exists.
  waitForLodash(() => {
    const setPropertyIdWarning = (message) => {
      const selector = 'label.warning[for="property-id"]';
      const warningLabel = $(selector);
      if (warningLabel.length) {
        warningLabel.text(message);
      }
    };

    const propertyIdField = $('#property-id');
    if (propertyIdField.length) {
      const propertyIdFieldDebounced = _.debounce(function () {
        const propertyId = propertyIdField.val();
        if (propertyId && propertyId.trim().length > 0) {
          const urlQuery = location.search.split('id=');
          const post_id = urlQuery.length > 1 ? urlQuery[1] : null;

          const additionalData = {};
          if (post_id) {
            additionalData.post_id = post_id;
          }
          ...
          $.ajax({
            ...
            type: 'post',
            data: {
              action: 'dirabe_realhomes_property_id_duplicate_search',
            ...
            success: function (response) {
              if (!response.success) {
                ...
                if (message && message.length > 0) {
                  setPropertyIdWarning(message);
                }
              } else {
                // Clean up any previous error messages.
                setPropertyIdWarning('');
              }
            }
          });
        } else {
      ...
      propertyIdField.on('input', propertyIdFieldDebounced);
    }
  }, 5000);
})(jQuery);