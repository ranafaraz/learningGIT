...
if (!function_exists(('rhea_min_floors_list'))) {
	function rhea_min_floors_list($placeholder)
	{
		$floors_array = [];
		for ($i = -1; $i <= 20; $i++) {
			$floors_array[] = $i;
		...
		$minimum_floor = '';
		if (isset($_GET['min-floor'])) {
			$minimum_floor = doubleval($_GET['min-floor']);
		}

		if (!empty($placeholder)) {
			echo '<option value="' . esc_attr(rhea_any_value()) . '" selected="selected">' . esc_html($placeholder) . '</option>';
		} else {
			echo '<option value="' . esc_attr(rhea_any_value()) . '">' . esc_html__('Min Floors', 'realhomes-elementor-addon') . '</option>';
		}

		if (!empty($floors_array)) {
			foreach ($floors_array as $floor) {
				if ($minimum_floor === $floor) {
					echo '<option value="' . $floor . '" selected="selected">' . $floor . '</option>';
				} else {
...
if (!function_exists(('rhea_max_floors_list'))) {
	function rhea_max_floors_list($placeholder)
	{
		$floors_array = [];
		for ($i = -1; $i <= 20; $i++) {
			$floors_array[] = $i;
		...
		$maximum_floor = '';
		if (isset($_GET['max-floor'])) {
			$maximum_floor = doubleval($_GET['max-floor']);
		}

		if (!empty($placeholder)) {
			echo '<option value="' . esc_attr(rhea_any_value()) . '" selected="selected">' . esc_html($placeholder) . '</option>';
		} else {
			echo '<option value="' . esc_attr(rhea_any_value()) . '">' . esc_html__('Max Floors', 'realhomes-elementor-addon') . '</option>';
		}

		if (!empty($floors_array)) {
			foreach ($floors_array as $floor) {
				if ($maximum_floor === $floor) {
					echo '<option value="' . $floor . '" selected="selected">' . $floor . '</option>';
				} else {
					echo '<option value="' . $floor . '">' . $floor . '</option>';
				}
	...
	function rhea_generate_options(array $hierarchical_terms_array, $searched_terms, string $prefix = '', $excluded_terms = null, $default = '')
	{
		if (!empty($hierarchical_terms_array)) {
			foreach ($hierarchical_terms_array as $term) {
				...
				if (
					!empty($searched_terms) && ((is_string($searched_terms) && $searched_terms == $term['slug'])
					...
					!empty($default) && ((is_string($default) && $default == $term['slug'])
...
if (!function_exists('rhea_generate_fields_icons')) {
	/**
	 ...
	 */
	function rhea_generate_fields_icons($icon_key, $settings)
	{
...
if (!function_exists('rhea_add_fields_icon_class')) {
	/**
	 ...
	 * @param string $icon_key
	 */
	function rhea_add_fields_icon_class($class_key, $settings)
	{
